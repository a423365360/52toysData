...
public class StockProductLine implements ExcelSheet {
    SXSSFWorkbook xssfWorkbook;
    Connection hiveConnection;

    public StockProductLine(SXSSFWorkbook xssfWorkbook, Connection hiveConnection) {
    ...
    public void setSheet(String table, String sql) throws Exception {
        ...
        PreparedStatement ps = this.hiveConnection.prepareStatement(sql);
        ResultSet resultSet = ps.executeQuery();
        sheet.createRow(0).createCell(0).setCellValue(table);

        SXSSFRow rowField = sheet.createRow(1);
        rowField.createCell(0).setCellValue("产品线");
        ...
        while (resultSet.next()) {
            SXSSFRow row = sheet.createRow(count3);
            row.createCell(0).setCellValue(resultSet.getString("product_line"));
