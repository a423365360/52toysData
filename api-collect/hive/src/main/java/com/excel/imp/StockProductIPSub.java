...
public class StockProductIPSub implements ExcelSheet {
    SXSSFWorkbook xssfWorkbook;
    Connection hiveConnection;

    public StockProductIPSub(SXSSFWorkbook xssfWorkbook, Connection hiveConnection) {
    ...
    public void setSheet(String table, String sql) throws Exception {
        ...
        rowField.createCell(0).setCellValue("IP细分");
        ...
        while (resultSet.next()) {
            SXSSFRow row = sheet.createRow(count4);
            row.createCell(0).setCellValue(resultSet.getString("ip_sub"));
